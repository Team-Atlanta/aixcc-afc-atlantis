...
def get_parser():  # pylint: disable=too-many-statements,too-many-locals
  ...
  execute_parser = subparsers.add_parser(
      'execute', help='Run a command within the builder container.')
  execute_parser.add_argument('project',
                              help='name of the project or path (external)')
  execute_parser.add_argument('source_path',
                              help='path of local source',
                              nargs='?')
  execute_parser.add_argument('--exec', 
                            help='command to execute',
                            default='/bin/bash',
                            type=str
                            )
  execute_parser.add_argument('--docker_image_tag',
                            dest='docker_image_tag',
                            default='latest',
                            help='docker image build tag'
                            'default: latest')
  _add_architecture_args(execute_parser)
  _add_engine_args(execute_parser)
  _add_sanitizer_args(execute_parser)
  _add_environment_args(execute_parser)
  _add_external_project_args(execute_parser)
...
def shell(args):
  ...
  extra_docker_args = os.getenv('OSS_FUZZ_EXTRA_DOCKER_ARGS')
  if extra_docker_args:
    run_args.extend(shlex.split(extra_docker_args))
  ...
  if hasattr(args, 'exec') and args.exec is not None:
    exec_command = ["sh", "-c", args.exec]
  else:
    exec_command = ['/bin/bash']

  run_args.extend(exec_command)
